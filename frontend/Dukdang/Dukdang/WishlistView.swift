...
struct WishlistView: View {
    ...
    // 💡 카테고리 필터링 로직 (MainView 방식 적용)
    var filteredItems: [WishTradePostItem] {
        if selectedCategory == "전체" {
            return wishItems
        } else {
            // 선택된 한글 카테고리에 맞는 영문 백엔드 카테고리 찾기
            let targetBackendCategory = categoryMap[selectedCategory] ?? "ETC"
            // 백엔드 카테고리(영문)와 데이터의 카테고리(영문) 비교
            return wishItems.filter { $0.category == targetBackendCategory }
        }
    }
