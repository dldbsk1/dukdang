...
struct ProductBar: View {
    let title: String
    let price: String
    let imageUrl: String? // 💡 [추가됨] 상품 이미지 주소
    
    var body: some View {
        HStack(spacing: 12) {
            // 💡 [핵심 수정] 기본 이모티콘 말고 진짜 상품 사진 띄우기!
            if let urlString = imageUrl, urlString.hasPrefix("http") {
...
struct ChatitemView: View {
    ...
    var productImageUrl: String? = nil // 💡 [추가됨]
    let sellerId: Int
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            // 💡 [수정됨] 상품 이미지 URL 넘겨주기
            ProductBar(title: productTitle, price: productPrice, imageUrl: productImageUrl)
...
// 프리뷰 (테스트용)
#Preview {
    ChatitemView(roomId: 1, productTitle: "테스트 상품", productPrice: "10,000원", productImageUrl: nil, sellerId: 1)
}
