...
struct ContentView: View {
    ...
    @State private var realImageUrls: [String] = [] // 💡 진짜 여러장 사진 배열
    
    @Environment(\.dismiss) private var dismiss
    
    let item: TradeItem

    // 💡 화면에 보여줄 사진 배열을 안전하게 변수로 분리!
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            .tabViewStyle(PageTabViewStyle()) // 동그라미 페이지네이션 스타일
            .frame(height: 300)
            .overlay( // 뒤로가기 버튼 등 덮어씌우기
                HStack {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 12) {
                    // 진짜 판매자 이름 반영
                    SellerProfileView(seller: realSellerName.isEmpty ? item.seller : TradeSellerInfo(name: realSellerName, location: "동네 미설정", profileImageName: "person.circle", temperature: 36.5))
        ...
        .navigationDestination(isPresented: $isGoChat) {
            if let roomId = selectedRoomId {
                // 💡 ChatitemView가 프로젝트에 정의되어 있어야 에러가 안 납니다!
                ChatitemView(
                    roomId: roomId,
...
struct SellerProfileView: View {
    ...
    var body: some View {
        HStack {
            Image(seller.profileImageName)
                .resizable()
                .frame(width: 40, height: 40)
                .clipShape(Circle())
            VStack(alignment: .leading) {
                Text(seller.name).font(.headline)
                Text(seller.location).font(.caption).foregroundColor(.gray)
            }
            Spacer()
            VStack(alignment: .trailing) {
                Text("\(seller.temperature, specifier: "%.1f")°C").foregroundColor(.blue).bold()
